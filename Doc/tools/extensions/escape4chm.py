...
def _process(string):
    def escape(matchobj):
        ...
        return '&#%d;' % codepoint if name is None else '&%s;' % name
...
def fixup_keywords(app, exception):
    ...
    with app.builder.open_file(outdir, f'{outname}.hhk', 'r') as f:
        index = f.read()
    with app.builder.open_file(outdir, f'{outname}.hhk', 'w') as f:
